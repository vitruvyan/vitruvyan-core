...
- `plugin`: domain extension loaded without changing core internals.
...
| Entity resolution | `entity_resolver_node` + `EntityResolverRegistry` | `domains.<domain>.entity_resolver_config.register_<domain>_entity_resolver()` | Same as above | Auto-registration in `graph_flow.py` using `ENTITY_DOMAIN` (registration); execution uses `ENTITY_DOMAIN` in `entity_resolver_node.py` |
...
| Babel Gardens | Signal domain primitives and YAML loader | `signals_<domain>.yaml` + optional service plugin | `vitruvyan_core/core/cognitive/babel_gardens/` + `services/api_babel_gardens/plugins/` | `load_config_from_yaml(...)` exists; service-level domain plugin loading is manual |
...
`graph_flow.py` reads both if present.
...
- Confirm no warning about missing `domains.security.intent_config`.
...
### SHOULD (operational hooks currently used in runtime)

- `entity_resolver_config.py`
- `execution_config.py`
- `graph_nodes/registry.py` (optional graph extension hook)

These files are used by current LangGraph hook pattern (`ENTITY_DOMAIN`, `EXEC_DOMAIN`, `GRAPH_DOMAIN`), even if not listed in the baseline SHOULD list of the contract.
...
### Nodi core vs logica di dominio (chiarimento normativo)

- `vitruvyan_core/core/orchestration/langgraph/node/` e riservato ai nodi core domain-agnostic.
- Il comportamento domain-specific deve vivere in `vitruvyan_core/domains/<domain>/...`.
- Nell'architettura corrente, la logica dominio si aggancia ai nodi core tramite moduli hook e registry, non modificando i nodi core per dominio.
...
Startup pattern (from `services/api_graph/README.md` and finance hook doc):
...
Compatibility fields from template are used by update-manager flow (`min_core_version`, `max_core_version`, `contracts_major`, `update_channel`).

### Validation options

- Update-manager validator available in repo:
  - `vitruvyan_core/core/platform/update_manager/ci/contract_validator.py`
- JSON schema file exists:
  - `docs/contracts/verticals/schema/vertical_manifest.schema.json`

No dedicated repo command was found that automatically validates vertical manifests against that JSON schema in current startup/runtime flow; use schema validation explicitly in CI if needed.
...
If your CI uses update-manager validator, align manifest status with that validator policy or extend the validator policy first.

## Sacred Orders status

Detailed order-by-order guide: `docs/knowledge_base/development/verticals/Vertical_Sacred_Orders.md`

Implementation status summary:

- Orthodoxy domain rules hook: implemented hook, startup wiring missing.
- Vault signal timeseries: adapter-level integration exists; not exposed as dedicated HTTP endpoints in current API routes.
- Babel signals YAML + plugin pattern: supported primitives, manual integration path.
...
| Extension point | File da creare (path) | Env var / config | Loader / wiring | Status | Contratto di riferimento |
|---|---|---|---|---|---|
...
| Entity | `vitruvyan_core/domains/<domain>/entity_resolver_config.py` with `register_<domain>_entity_resolver()` | `ENTITY_DOMAIN` | Auto-register in `graph_flow.py`, execution via `entity_resolver_node.py` + `EntityResolverRegistry` | `ACTIVE` (set `ENTITY_DOMAIN` explicitly) | `docs/contracts/verticals/VERTICAL_CONTRACT_V1.md` |
...
| Babel | `signals_<domain>.yaml` (+ optional service plugin in `services/api_babel_gardens/plugins/`) | plugin path/config path (manual) | `load_config_from_yaml(...)` in Babel domain layer; service plugin loading is manual | `EXPERIMENTAL` | `docs/contracts/verticals/VERTICAL_CONTRACT_V1.md` |
...
| Codex | domain pack files (for example `examples/verticals/<domain>/...`) + optional graph routing hook | config YAML via `CodexConfig.from_yaml(...)` (manual) | Core supports config objects; service/domain-pack auto-loading not wired | `EXPERIMENTAL` | `docs/contracts/verticals/VERTICAL_CONTRACT_V1.md` |
...
- Ensure importability of `domains` package (PYTHONPATH/runtime path).
...
### Pattern taxonomy YAML not loaded

- `PATTERN_TAXONOMY_PATH` alone is not enough.
- Service startup must build `PatternConfig.from_env()` and inject via `set_config(...)` into pattern-weavers domain config.

### Babel signals YAML/plugin not loaded
