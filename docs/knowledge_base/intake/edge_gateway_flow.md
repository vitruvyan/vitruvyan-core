...
Describe how edge runtimes ingest local data, buffer offline, and relay to Core Intake.

## Flow Graph

```mermaid
flowchart TD
    A[Edge Source<br/>H2M or M2M] --> B[Edge Gateway API<br/>POST /api/edge/intake]
    B --> C[(SQLite Outbox<br/>status=pending)]
    C --> D{Core Reachable?}
    D -- Yes --> E[Relay to Core Intake<br/>/api/intake/{type}]
    E --> F[(Core PostgreSQL<br/>evidence_packs)]
    E --> G[[Redis Streams<br/>intake.evidence.created]]
    D -- No --> H[Keep Pending]
