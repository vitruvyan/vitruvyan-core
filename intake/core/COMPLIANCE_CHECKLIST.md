...
| ID | Requirement | Status | Evidence | Notes |
|----|-------------|--------|----------|-------|
| 4.1 | Events have `schema_ref` = `aegis://intake/events/evidence_created/v1.0` | ✅ | `event_evidence_created_v1.json` line 15 | Schema enforces constant |
| 4.2 | `idempotency_key` = SHA-256(evidence_id + chunk_id + source_hash) | ✅ | `event_emitter.py` lines 80-95 | Prevents duplicate processing |
| 4.3 | Events emitted to Redis channel `intake.evidence.created` | ✅ | `event_emitter.py` line 120 | Redis pub/sub enforced |
