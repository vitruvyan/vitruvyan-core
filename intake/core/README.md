...
│  │  │  - Redis pub/sub (intake.evidence.created)       │ │  │
│  │  │  - Audit logging (success/failure)               │ │  │
│  │  └──────────────────────────────────────────────────┘ │  │
│  └────────────────────────────────────────────────────────┘  │
└──────────────────────────────────────────────────────────────┘
                          │
                          │ intake.evidence.created event
                          ▼
┌──────────────────────────────────────────────────────────────┐
...
```
vitruvyan_core/aegis/intake/
├── README.md                                   ← This file
├── ACCORDO-FONDATIVO-INTAKE-V1.1.md            ← Canonical specification
...
├── event_evidence_created_v1.json              ← Event schema for intake.evidence.created
...
```python
from vitruvyan_core.aegis.intake.agents.document_intake import DocumentIntakeAgent
from vitruvyan_core.aegis.intake.event_emitter import IntakeEventEmitter
from vitruvyan_core.core.foundation import PostgresAgent, RedisClient
...
```python
from vitruvyan_core.aegis.intake.agents.image_intake import ImageIntakeAgent
...
evidence_ids = agent.ingest_image(
...
print(f"Created Evidence Pack: {evidence_ids[0]}")
```
...
```python
from vitruvyan_core.aegis.intake.agents.audio_intake import AudioIntakeAgent
...
```python
from vitruvyan_core.aegis.intake.agents.video_stream_intake import VideoStreamIntakeAgent
...
**Event**: `intake.evidence.created`  
**Schema URI**: `aegis://intake/events/evidence_created/v1.0`
...
**A**: Codex subscribes to Redis channel `intake.evidence.created`. When event arrives, Codex retrieves Evidence Pack from PostgreSQL and performs enrichment.
...
- [Event Schema: intake.evidence.created v1.0](event_evidence_created_v1.json)
