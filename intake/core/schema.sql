...
CREATE TABLE IF NOT EXISTS intake_event_log (
    ...
    schema_ref VARCHAR(255) NOT NULL,  -- e.g., "aegis://intake/events/evidence_created/v1.0"
...
-- Comment
COMMENT ON TABLE intake_event_log IS 'Audit trail for successfully emitted intake.evidence.created events';
COMMENT ON COLUMN intake_event_log.idempotency_key IS 'SHA-256 hash: prevents duplicate event emission';
COMMENT ON COLUMN intake_event_log.schema_ref IS 'Event schema URI (e.g., aegis://intake/events/evidence_created/v1.0)';
...
-- CREATE ROLE aegis_intake_user WITH LOGIN PASSWORD 'secure_password';
-- GRANT CONNECT ON DATABASE vitruvyan TO aegis_intake_user;
-- GRANT USAGE ON SCHEMA public TO aegis_intake_user;
-- GRANT SELECT, INSERT ON evidence_packs TO aegis_intake_user;
-- GRANT SELECT, INSERT ON intake_event_log TO aegis_intake_user;
-- GRANT SELECT, INSERT ON intake_event_failures TO aegis_intake_user;
-- GRANT SELECT ON evidence_packs_recent TO aegis_intake_user;
-- GRANT SELECT ON intake_event_log_recent TO aegis_intake_user;
-- GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO aegis_intake_user;

-- Revoke dangerous permissions
-- REVOKE UPDATE, DELETE, TRUNCATE ON evidence_packs FROM aegis_intake_user;
-- REVOKE UPDATE, DELETE, TRUNCATE ON intake_event_log FROM aegis_intake_user;
-- REVOKE UPDATE, DELETE, TRUNCATE ON intake_event_failures FROM aegis_intake_user;

COMMENT ON SCHEMA public IS 'RBAC enforced: application user has INSERT+SELECT only on Intake tables';
...
CREATE TABLE IF NOT EXISTS geospatial_evidence (
    ...
    CONSTRAINT fk_geospatial_evidence_pack
        ...
        REFERENCES evidence_packs(evidence_id)
        ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS landscape_evidence (
    ...
    FOREIGN KEY (evidence_id) REFERENCES evidence_packs(evidence_id) ON DELETE CASCADE
);
