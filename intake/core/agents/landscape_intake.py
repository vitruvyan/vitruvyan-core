    ...
    def ingest(
        ...
        evidence_pack = {
            ...
            "integrity": {
                "hash": integrity_hash,
                "algorithm": "sha256"
            },  # TOP-LEVEL (required by schema v0.1.11)
        ...
        self.event_emitter.emit_evidence_created(
            ...
            chunk_id="chunk_001",  # Default chunk for single-fragment evidence
            source_type=evidence_pack["source_type"],
            source_uri=evidence_pack["file_reference"],
            evidence_pack_ref=evidence_pack["evidence_id"],  # Reference to persisted pack
            source_hash=evidence_pack["integrity"]["hash"],
    ...
    def _persist_evidence(self, evidence_pack: Dict[str, Any]):
        ...
        try:
            with self.postgres_agent.connection.cursor() as cur:
                ...
                integrity = {
                    "hash": evidence_pack["integrity"]["hash"],
                    "algorithm": "sha256"
                }
                ...
                """, (
                    ...
                    "chunk_001",  # Default chunk for single-fragment evidence
                    evidence_pack["created_at"],
                    json.dumps(source_ref),
                    evidence_pack["literal_text"],
                ...
                """, (
                    ...
                    evidence_pack["created_at"],
                    evidence_pack["integrity"]["hash"]  # Audit trail (source hash = integrity hash)
                ))
