...
class ShadowTradersCognitiveBusListener:
    ...
    def __init__(self, redis_url: str = None):
        ...
        self.redis_url = redis_url or os.getenv('REDIS_URL', 'redis://mercator_redis:6379')
    ...
    async def publish_order_executed(self, order_data: Dict[str, Any]):
        ...
        try:
            ...
            # Primary path: Redis Streams (Mercator bus)
            self.stream_bus.emit(
                channel=channel,
                payload=event,
                emitter="shadow_traders.api",
                correlation_id=str(order_data.get("order_id", "")) or None,
            )
            # Compatibility path: pub/sub if local listener connection exists.
    ...
    async def publish_portfolio_updated(self, portfolio_data: Dict[str, Any]):
        ...
        try:
            ...
            self.stream_bus.emit(
                ...
                payload=event,
                emitter="shadow_traders.api",
                correlation_id=str(portfolio_data.get("user_id", "")) or None,
            )
            if self.redis_client:
                await self.redis_client.publish(channel, json.dumps(event))
    ...
    async def publish_risk_alert(self, risk_data: Dict[str, Any]):
        ...
        try:
            ...
            self.stream_bus.emit(
                ...
                payload=event,
                emitter="shadow_traders.api",
                correlation_id=str(risk_data.get("user_id", "")) or None,
            )
            if self.redis_client:
                await self.redis_client.publish(channel, json.dumps(event))
